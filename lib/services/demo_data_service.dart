...
class DemoDataService {
  ...
  static Future<void> _createDemoUsers() async {
    ...
    final existingUsers = await UserService.getAllUsers();
    ...
    await UserService.createUser(participant);
    await UserService.createUser(organizer);
    await UserService.setCurrentUser(participant);
  }

  static Future<void> _createDemoEvents() async {
    // Check if events already exist
    final existingEvents = await EventService.getAllEvents();
    if (existingEvents.isNotEmpty) return;

    final now = DateTime.now();
    final organizerId = 'organizer_1';
